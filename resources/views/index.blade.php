...
{{'@'}}section('title', trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.index'))
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.index') }}
@if($export)
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="{{'{{'}} url('admin/{{ $resource }}/export') }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.export') }}</a>
@endif
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{'{{'}} url('admin/{{ $resource }}/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.create') }}</a>
                    </div>
...
@if($col['name'] === 'published_at')
                                        <th is='sortable' class="text-center" :column="'{{ $col['name'] }}'">{{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</th>
@else                                        <th is='sortable' :column="'{{ $col['name'] }}'">{{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</th>
@endif
@endforeach
...
@elseif($col['name'] === 'published_at')    <td class="text-center text-nowrap">
                                            <span v-if="item.published_at <= now">
                                                ...
                                                <span v-if="item.published_at > now">
                                                <small>{{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.will_be_published') }}</small><br />
                                                {{'@{{'}} item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{'{{'}} url('admin/{{ $resource }}/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
