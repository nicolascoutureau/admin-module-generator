...
{{'@'}}section('title', trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.edit', ['name' => ${{ $modelVariableName }}->{{$modelTitle}}]))
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-header">
                                    <i class="fa fa-pencil"></i> {{'{{'}} trans('{{$moduleName}}::admin.{{ $modelLangFormat }}.actions.edit', ['name' => ${{ $modelVariableName }}->{{$modelTitle}}]) }}
                                </div>
                                <div class="card-body">
                                    {{'@'}}include('{{$moduleName}}::admin.{{ $modelDotNotation }}.components.form-elements')
                                </div>
                            </div>
                        </div>

                        <div class="col-md-12 col-lg-12 col-xl-5 col-xxl-4">
                            {{'@'}}include('{{$moduleName}}::admin.{{ $modelDotNotation }}.components.form-elements-right', ['showHistory' => true])
                        </div>
                    </div>
                    @else
